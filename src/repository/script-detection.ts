...
const TEST_SIGNAL =
  /(?:^|[\s;&|])(?:node\s+--test(?:\s|$)|bun\s+test(?:\s|$)|deno\s+test(?:\s|$)|mocha(?:\s|$)|ava(?:\s|$)|tap(?:\s|$)|tape(?:\s|$)|uvu(?:\s|$)|playwright\s+test(?:\s|$)|cypress\s+run(?:\s|$)|(?:npm|pnpm|yarn|bun)\s+(?:run\s+)?(?:test(?:[:_-][\w.-]+)?|["']?\/\^test))/;
