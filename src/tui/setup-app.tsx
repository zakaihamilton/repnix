...
import type { HealthFinding, InstallPlan, RepositoryDiagnostic } from "../core/types.js";
...
import {
  ...
  ConfirmView,
  DetailsView,
  ManualRecommendationsView,
  ReviewView,
  SelectView,
  auditUsesSingleColumn,
  AUDIT_LABEL_COLUMN_WIDTH,
  AUDIT_TWO_COLUMN_MIN_WIDTH,
  checkDetailViewport,
  parseHealthRun,
  setupCheckActions,
  setupCheckCommand,
  setupCheckDetailItems,
  setupCheckRows,
} from "./setup-views.js";
  ...
  const onOutput = (stream: "stdout" | "stderr", chunk: Buffer) => {
    ...
    for (const line of lines) {
      try {
        ...
        if (event.event === "health.provider.start" || event.event === "health.provider.finish")
          onProgress?.(event.message ?? "Check progress updated");
      } catch {
...
export function setupCheckResultOutput(
  check: Pick<CommandResult, "command" | "args" | "exitCode" | "signal" | "stdout" | "stderr" | "spawnError">,
): string {
...
function appendCheckResultsTable(lines: string[], rows: ReturnType<typeof setupCheckRows>): void {
  ...
  for (const row of rows)
    lines.push(
      `| ${checkStatusLabelForFile(row.status)} | ${markdownCell(row.category)} | ${markdownCell(row.result)} | ${markdownCell(row.providers)} |`,
    );
}
...
export function renderSetupHealthReport(output: string): string {
  ...
  if (!run)
    return [
      ...
      "",
      "The health check did not produce structured results, so RepNix could not create an AI-ready report.",
      "",
      "Run `repnix check` again and include its output when asking for help.",
      "",
    ].join("\n");
  ...
  run.results.forEach((result, resultIndex) => {
    ...
    lines.push(
      "",
      `### ${resultIndex + 1}. ${markdownCell(category)} — ${markdownCell(result.name)}`,
      "",
      `- Status: ${checkStatusLabelForFile(result.status)}`,
      `- Run: \`${markdownCode(command)}\``,
    ...
    result.findings.forEach((finding, findingIndex) => {
      lines.push(
        "",
        `#### Finding ${findingIndex + 1}: ${markdownCell(finding.title ?? finding.ruleId ?? finding.type)}`,
        "",
        `- Severity: ${finding.severity}`,
        `- Message: ${markdownCell(finding.message)}`,
      );
...
export function SetupApp({ options, logger, dependencies = {}, result }: SetupTuiProps): React.ReactElement {
  ...
  const [model, setModel] = useState<SetupTuiModel>({
    screen: "loading",
    cursor: 0,
    auditScroll: 0,
    manualScroll: 0,
  ...
  const dispatch = (action: Parameters<typeof setupTuiReducer>[1]) =>
    setModel((current) => setupTuiReducer(current, action));
  ...
  useEffect(() => {
    void deps
      .audit(process.cwd(), { ...options, logger })
      .then((nextAudit) => {
        setAudit(nextAudit);
  ...
  useEffect(() => {
    ...
    void deps
      .buildPlan(audit.context, model.selectedProviders, model.includeCi, audit.registry)
      .then((nextPlan) => {
        setPlan(nextPlan);
  ...
  useEffect(() => {
    ...
    void deps
      .runCheck(
        ...
        (message) => dispatch({ type: "check-progress", message }),
      )
      .then((check) => {
        const output = setupCheckResultOutput(check);
        return saveSetupCheckReports(audit.context.root, check.stdout).then((saved) =>
          dispatch({ type: "check-complete", output, exitCode: check.exitCode, ...(saved ? saved : {}) }),
        );
      })
      .catch((error: unknown) =>
        dispatch({
    ...
    void deps
      ...
      .then(async () => {
        ...
        dispatch({
          type: "progress",
          message: gained
            ? `${gained} additional categor${gained === 1 ? "y is" : "ies are"} now covered.`
            : "Setup changes were verified.",
        });
  ...
  const checkLineCount = setupCheckDetailItems(model.checkOutput ?? "", width, {
  ...
  const busy =
    ...
    model.screen === "planning" ||
    model.screen === "applying" ||
  ...
  useInput((input, key) => {
    ...
    if (model.screen === "audit") {
      if (key.upArrow || input === "k")
        ...
        dispatch({
          type: audit?.recommendations.some((recommendation) => !recommendation.actionable)
            ? "begin-manual"
            : audit?.recommendations.some((recommendation) => recommendation.actionable)
              ? "begin-selection"
              : "show-empty",
        });
      return;
    ...
    if (model.screen === "manual") {
      if (key.upArrow || input === "k")
        dispatch({
          type: "move-manual",
          direction: "up",
          lineCount: manualLineCount,
          viewport: manualRecommendationViewport(layout.detailViewport),
        });
      else if (key.downArrow || input === "j")
        dispatch({
          type: "move-manual",
    ...
    if (model.screen === "success") {
      dispatch({ type: "begin-check" });
      return;
    }
    if (model.screen === "empty" || model.screen === "error") {
      result.code = model.screen === "error" ? 2 : 0;
      exit();
      return;
    }
    if (model.screen === "select") {
      ...
      if (sidebarFocused && (key.upArrow || input === "k"))
        dispatch({ type: "move", direction: "up", itemCount: items.length });
      else if (sidebarFocused && (key.downArrow || input === "j"))
        dispatch({ type: "move", direction: "down", itemCount: items.length });
      else if (sidebarFocused && input === " ") {
        const item = items[model.cursor];
    ...
    if (model.screen === "review") {
      const sidebarFocused = sidebarMode ? !model.sidebarCollapsed : true;
      if (sidebarFocused && (key.upArrow || input === "k"))
        dispatch({ type: "move-review", direction: "up", fileCount: plan?.files.length ?? 0 });
      else if (sidebarFocused && (key.downArrow || input === "j"))
        dispatch({ type: "move-review", direction: "down", fileCount: plan?.files.length ?? 0 });
    ...
    if (model.screen === "details") {
      if (key.upArrow || input === "k")
        dispatch({ type: "move-detail", direction: "up", lineCount: detailLineCount, viewport: layout.detailViewport });
      else if (key.downArrow || input === "j")
        dispatch({
          type: "move-detail",
          direction: "down",
          lineCount: detailLineCount,
          viewport: layout.detailViewport,
        });
    }
    if (model.screen === "check-details") {
      if (key.upArrow || input === "k")
        dispatch({ type: "move-check", direction: "up", lineCount: checkLineCount, viewport: checkViewport });
      else if (key.downArrow || input === "j")
        dispatch({ type: "move-check", direction: "down", lineCount: checkLineCount, viewport: checkViewport });
    }
  });
  ...
  return (
    <Box flexDirection="column" width="100%" height={height} paddingX={1} overflow="hidden">
      <Header
        model={model}
        repositoryName={audit?.context.packageJson.name ?? "repository"}
        packageManager={audit?.context.packageManager ?? null}
        compact={compact}
        theme={theme}
      />
      <Box flexDirection="column" flexGrow={1} flexShrink={1} minHeight={0} overflow="hidden">
        {model.screen === "loading" ? (
          <Panel title="Scanning repository" theme={theme} borderColor={theme.info}>
            <Text color={theme.info}>◌ Detecting checks, project structure, and recommendations…</Text>
          </Panel>
        ) : null}
        {model.screen === "audit" && audit ? (
          <AuditView
            audit={audit}
            singleColumn={auditSingleColumn}
            scroll={model.auditScroll}
            viewport={layout.detailViewport}
            theme={theme}
          />
        ) : null}
        {model.screen === "manual" && audit ? (
          <ManualRecommendationsView
            audit={audit}
            scroll={model.manualScroll}
            viewport={layout.detailViewport}
            width={width}
            theme={theme}
          />
        ...
        {model.screen === "empty" ? (
          <Panel title="Nothing to add" theme={theme} borderColor={theme.success}>
            <Text color={theme.success}>● Your active checks already cover the gaps RepNix found.</Text>
          </Panel>
        ) : null}
        {model.screen === "select" && audit ? (
          <SelectView audit={audit} model={model} paneLayout={paneLayout} theme={theme} />
        ) : null}
        {model.screen === "planning" ? (
          <Panel title="Preparing review" theme={theme} borderColor={theme.info}>
            <Text color={theme.info}>◌ Building a safe setup plan…</Text>
          </Panel>
        ) : null}
        {model.screen === "review" && plan ? (
          <ReviewView plan={plan} model={model} paneLayout={paneLayout} theme={theme} />
        ) : null}
        {model.screen === "details" && plan ? (
          <DetailsView plan={plan} model={model} width={width} layout={layout} theme={theme} />
        ) : null}
        {model.screen === "confirm" && audit && plan ? (
          <ConfirmView audit={audit} plan={plan} model={model} theme={theme} />
        ) : null}
        {model.screen === "applying" && plan ? <ApplyView plan={plan} model={model} theme={theme} /> : null}
        {model.screen === "checking" ? (
          <Panel title="Running check" theme={theme} borderColor={theme.info}>
            <Text color={theme.info} bold>
              ◌ Running repository health checks
            </Text>
            <Text color={theme.muted}>
              Checks may run in parallel. Completed results will appear when all checks finish.
            </Text>
            <Newline />
            <Text color={theme.secondary} bold>
              LIVE ACTIVITY
            </Text>
            {(model.checkProgress ?? ["Preparing configured checks…"]).map((message, index, entries) => (
              <Text key={`${index}-${message}`} color={index === entries.length - 1 ? theme.primary : theme.muted}>
                {index === entries.length - 1 ? "◌" : "✓"} {message}
              </Text>
            ))}
          </Panel>
        ) : null}
        {model.screen === "success" ? (
          <Panel title="Setup complete" theme={theme} borderColor={theme.success}>
            <Text color={theme.success}>● Repository health setup completed successfully.</Text>
            <Newline />
            <Text>{model.progress}</Text>
            <Text color={theme.primary} bold>
              Press Enter to run `repnix check` now.
            </Text>
            <Text color={theme.muted}>Run a category with --details later when you need more context.</Text>
          </Panel>
        ) : null}
        {model.screen === "check-details" ? (
          <CheckDetailsView model={model} width={width} layout={layout} theme={theme} />
        ) : null}
        {model.screen === "error" ? (
          <Panel title="Setup stopped" theme={theme} borderColor={theme.danger}>
            <Text color={theme.danger}>◆ {model.error ?? "An unexpected error occurred."}</Text>
            <Newline />
            <Text color={theme.muted}>No further changes will be applied.</Text>
          </Panel>
        ) : null}
      </Box>
      <Footer
        ...
        hasManualRecommendations={Boolean(audit?.recommendations.some((recommendation) => !recommendation.actionable))}
        theme={theme}
      />
    </Box>
  );
...
export {
  ...
  setupCheckActions,
  setupCheckDetailItems,
  setupCheckOutputLines,
  setupCheckRows,
  tuiPanelContentWidth,
} from "./setup-views.js";
