...
export async function checkCommand(category: string | undefined, options: CheckOptions): Promise<number> {
  if (options.format && !["summary", "details", "json", "sarif"].includes(options.format)) throw new Error(`Unknown check format '${options.format}'. Use summary, details, json, or sarif.`);
  ...
  const baseline = config.baseline && !options.writeBaseline ? await readBaseline(audit.context.root, config.baseline.path) : undefined;
