...
function severityFor(vulnerability: Record<string, unknown>): FindingSeverity {
  ...
  const label =
    database && typeof database === "object" && "severity" in database
...
function fixedVersions(vulnerability: Record<string, unknown>): string[] {
  ...
  for (const item of affected) {
    ...
    const ranges = Array.isArray((item as Record<string, unknown>).ranges)
      ? ((item as Record<string, unknown>).ranges as unknown[])
      : [];
    for (const range of ranges) {
      if (!range || typeof range !== "object") continue;
      const events = Array.isArray((range as Record<string, unknown>).events)
        ? ((range as Record<string, unknown>).events as unknown[])
        : [];
...
export function normalizeOsv(report: unknown): HealthFinding[] {
  ...
  for (const result of (report as { results: unknown[] }).results) {
    ...
    const source =
      resultRecord.source && typeof resultRecord.source === "object"
        ? (resultRecord.source as Record<string, unknown>)
        : {};
    ...
    for (const entry of packages) {
      ...
      const packageData =
        entryRecord.package && typeof entryRecord.package === "object"
          ? (entryRecord.package as Record<string, unknown>)
          : {};
