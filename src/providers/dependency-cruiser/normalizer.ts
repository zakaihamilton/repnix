...
export function normalizeDependencyCruiser(report: unknown): HealthFinding[] {
  if (!report || typeof report !== "object") throw new Error("dependency-cruiser JSON report has an unsupported shape.");
  ...
  for (const violation of (summary as { violations: unknown[] }).violations) {
    ...
    const ruleName = typeof rule.name === "string" ? rule.name : typeof item.rule === "string" ? item.rule : "architecture-rule";
