...
export async function readBaseline(root: string, file: string): Promise<BaselineFile> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      throw new Error(`Configured RepNix baseline does not exist: ${file}. Run 'repnix check --write-baseline' after reviewing current findings.`);
    }
    if (error instanceof SyntaxError || error instanceof z.ZodError) {
      throw new Error(`Invalid RepNix baseline at ${file}: ${error instanceof z.ZodError ? z.prettifyError(error) : error.message}`);
    }
    throw error;
  }
}
...
export function baselineFromRun(run: HealthRun): BaselineFile {
  const entries = run.results.flatMap((result) => result.findings.map((finding) => ({
...
export async function writeBaseline(root: string, file: string, run: HealthRun): Promise<void> {
  if (run.summary.errors > 0) throw new Error("Cannot write a baseline while one or more providers failed to complete.");
  const baselinePath = path.resolve(root, file);
  if (path.relative(root, baselinePath).startsWith("..")) throw new Error("Baseline path must stay inside the repository.");
