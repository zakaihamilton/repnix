...
const schema = z
  .object({
    ...
    scopes: z.record(z.string(), z.object({ roles: z.array(repositoryRole).min(1).optional(), categories: categories.optional() }).strict()).optional(),
    categories: categories.optional(),
    severityThreshold: z.enum(["info", "warning", "error"]).default("warning"),
    execution: z.object({ jobs: z.number().int().min(1).max(32).default(2), timeoutSeconds: z.number().positive().default(300) }).strict().default({ jobs: 2, timeoutSeconds: 300 }),
    baseline: z.object({ path: z.string().min(1).default(".repnix-baseline.json"), failOn: z.enum(["new", "all"]).default("new") }).strict().optional(),
    policies: z
      .object({
...
export function categoryModeFor(config: RepnixConfig, category: HealthCategory, scope?: string): "required" | "optional" | "off" | undefined {
  return (scope === undefined ? undefined : config.scopes?.[scope]?.categories?.[category]?.mode) ?? config.categories?.[category]?.mode;
}
  ...
  return {
    ...
    scopes: context.scopes.map((scope) => {
      ...
      return {
        ...
        roleEvidence: roles.map((role) => ({ role: role as RepositoryRole, confidence: "configured" as const, signals: ["repnix.config.json scope override"] })),
      };
    }),
  };
}
...
export function validateConfigCategories(config: RepnixConfig, availableCategories: Iterable<string>): void {
  ...
  if (unknown) throw new Error(`Unknown health category '${unknown}'. Available categories: ${[...available].join(", ")}`);
}
