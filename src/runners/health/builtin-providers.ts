...
import { commandResult, executableBinary, expectedLocalBinary, HEALTH_OFFLINE_ENV, localBinary, outputExcerpt, resolveLogger, statusForFindings, type RunnableCommand } from "./task-executor.js";

export type BuiltinRunner = (context: RepositoryContext, config: RepnixConfig, logger: DiagnosticLogger, timeoutMs?: number) => Promise<HealthResult>;

async function runCoveragePolicy(context: RepositoryContext, config: RepnixConfig, logger: DiagnosticLogger, timeoutMs?: number): Promise<HealthResult> {
  const testScript = safeTestScript(context.scripts);
  if (!testScript) return { provider: "c8", name: "c8", category: "coverage", status: "skipped", findings: [], durationMs: 0, message: "Coverage is detected, but no safe test command is configured." };
  ...
  if (config.policies?.coverage && Object.keys(config.policies.coverage).length) {
    ...
    for (const [key, value] of Object.entries(config.policies.coverage)) if (typeof value === "number") args.push(`--${key}`, String(value));
  }
  const testArgs = context.packageManager === "yarn" ? [testScript] : ["run", testScript];
  const runnable: RunnableCommand = { provider: "c8", name: "c8", category: "coverage", command: await expectedLocalBinary(context.root, "c8"), args: [...args, context.packageManager!, ...testArgs], ...(timeoutMs === undefined ? {} : { timeoutMs }) };
  const result = await runCommand(runnable.command, runnable.args, { cwd: context.root, logger, env: HEALTH_OFFLINE_ENV, ...(timeoutMs === undefined ? {} : { timeoutMs }) });
  return commandResult(runnable, result, context);
}

async function runLicensePolicy(context: RepositoryContext, config: RepnixConfig, logger: DiagnosticLogger, timeoutMs?: number): Promise<HealthResult> {
  ...
  const result = await runCommand(binary, ["--json"], { cwd: context.root, logger, env: HEALTH_OFFLINE_ENV, ...(timeoutMs === undefined ? {} : { timeoutMs }) });
  ...
  try {
    ...
    if (report && typeof report === "object") for (const [dependency, value] of Object.entries(report as Record<string, unknown>)) {
      const license = value && typeof value === "object" && typeof (value as Record<string, unknown>).licenses === "string" ? (value as Record<string, unknown>).licenses as string : "UNKNOWN";
      if (deny.has(license) || (allow.size > 0 && !allow.has(license))) findings.push(createFinding({ provider: "license-checker", category: "licenses", type: "license-policy", severity: "error", message: `${dependency} uses license ${license}, which is outside the configured license policy.`, metadata: { dependency, license } }));
    }
...
export async function runKnip(context: RepositoryContext, diagnostics: DiagnosticLogger | boolean, timeoutMs?: number): Promise<HealthResult> {
  ...
  try {
    ...
    return { provider: "knip", name: "Knip", category: "dead-code", status: statusForFindings(findings), findings, durationMs: result.durationMs };
  } catch (error) {
...
export async function runOsvScanner(context: RepositoryContext, diagnostics: DiagnosticLogger | boolean, timeoutMs?: number): Promise<HealthResult> {
  ...
  try {
    ...
    if (result.exitCode !== 0 && !findings.length) return { provider: "osv-scanner", name: "OSV-Scanner", category: "security", status: "error", findings: [], durationMs: result.durationMs, message: `OSV-Scanner could not complete its offline scan. ${outputExcerpt(result)}`.trim() };
...
export async function runDependencyCruiser(context: RepositoryContext, diagnostics: DiagnosticLogger | boolean, timeoutMs?: number): Promise<HealthResult> {
  ...
  try {
    ...
    if (result.exitCode !== 0 && !findings.length) return { provider: "dependency-cruiser", name: "dependency-cruiser", category: "architecture", status: "error", findings: [], durationMs: result.durationMs, message: `dependency-cruiser could not complete its analysis. ${outputExcerpt(result)}`.trim() };
...
async function runEslintBoundaries(context: RepositoryContext, _config: RepnixConfig, logger: DiagnosticLogger, timeoutMs?: number): Promise<HealthResult> {
  const lintScript = safeScript(context, ["lint", "check:lint", "lint:check"], "general");
  const command = lintScript ? scriptCommand(context, lintScript) : { command: await expectedLocalBinary(context.root, "eslint"), args: ["."] };
  ...
  return commandResult(runnable, await runCommand(runnable.command, runnable.args, { cwd: context.root, logger, env: { ...HEALTH_OFFLINE_ENV, ...runnable.env }, ...(timeoutMs === undefined ? {} : { timeoutMs }) }), context);
}
...
export async function runSizeLimit(context: RepositoryContext, diagnostics: DiagnosticLogger | boolean, timeoutMs?: number): Promise<HealthResult> {
  ...
  if (/size limit|limit.{0,30}(?:exceed|over)|(?:exceed|over).{0,30}limit/i.test(output)) return { provider: "size-limit", name: "Size Limit", category: "bundle", status: "fail", findings: [createFinding({ provider: "Size Limit", category: "bundle", type: "bundle-budget", severity: "error", message: "The configured bundle-size budget was exceeded.", metadata: { output } })], durationMs: result.durationMs };
...
export async function runPublint(context: RepositoryContext, diagnostics: DiagnosticLogger | boolean, timeoutMs?: number): Promise<HealthResult> {
  const logger = resolveLogger(diagnostics);
  if (!await localBinary(context.root, "publint")) return { provider: "publint", name: "Publint", category: "package-health", status: "error", findings: [], durationMs: 0, message: "Publint is declared but its local binary is unavailable. Install dependencies first." };
  ...
  try {
    ...
    const result = await runCommand(process.execPath, ["--input-type=module", "--eval", PUBLINT_EVAL, packed.tarball], { cwd: context.root, logger, env: HEALTH_OFFLINE_ENV, ...(timeoutMs === undefined ? {} : { timeoutMs }) });
    ...
    try {
      ...
      if (result.exitCode !== 0 && !findings.length) return { provider: "publint", name: "Publint", category: "package-health", status: "error", findings: [], durationMs, message: `Publint could not complete its analysis. ${outputExcerpt(result)}`.trim() };
      return { provider: "publint", name: "Publint", category: "package-health", status: statusForFindings(findings), findings, durationMs };
    } catch (error) { return { provider: "publint", name: "Publint", category: "package-health", status: "error", findings: [], durationMs, message: `${error instanceof Error ? error.message : String(error)} ${outputExcerpt(result)}`.trim() }; }
...
export async function runAttw(context: RepositoryContext, diagnostics: DiagnosticLogger | boolean, timeoutMs?: number): Promise<HealthResult> {
  ...
  try {
    ...
    const result = await runCommand(binary, [packed.tarball, "--format", "json", "--no-definitely-typed"], { cwd: context.root, logger, env: HEALTH_OFFLINE_ENV, maxOutputBytes: 50 * 1024 * 1024, ...(timeoutMs === undefined ? {} : { timeoutMs }) });
    const durationMs = packed.durationMs + result.durationMs;
    ...
    try {
      ...
      try {
        const raw = JSON.parse(await readFile(path.join(context.root, ".attw.json"), "utf8")) as Record<string, unknown>;
        if (Array.isArray(raw.ignoreRules) && raw.ignoreRules.every((rule) => typeof rule === "string")) options.ignoreRules = raw.ignoreRules as string[];
        if (raw.profile === "strict" || raw.profile === "node16" || raw.profile === "esm-only") options.profile = raw.profile;
      } catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") { /* ATTW reports malformed config. */ } }
      const findings = normalizeAttw(parseJsonOutput(result.stdout), options);
      if (result.exitCode !== 0 && !findings.length) return { provider: "attw", name: "Are The Types Wrong?", category: "package-health", status: "error", findings: [], durationMs, message: `Are The Types Wrong? could not complete its analysis. ${outputExcerpt(result)}`.trim() };
    ...
    } catch (error) { return { provider: "attw", name: "Are The Types Wrong?", category: "package-health", status: "error", findings: [], durationMs, message: `${error instanceof Error ? error.message : String(error)} ${outputExcerpt(result)}`.trim() }; }
  } finally { await rm(temporary, { recursive: true, force: true }); }
}
