...
export function scriptCommand(
  ...
  script: string,
): Pick<RunnableCommand, "command" | "args" | "env"> {
  const executable = process.platform === "win32" ? (process.env.ComSpec ?? "cmd.exe") : "/bin/sh";
  return {
    command: executable,
    args:
      process.platform === "win32" ? ["/d", "/s", "/c", context.scripts[script]!] : ["-c", context.scripts[script]!],
    env: {
      CI: "1",
...
function workspaceScriptCommand(
  context: RepositoryContext,
  workspace: string,
  script: string,
): Pick<RunnableCommand, "command" | "args" | "env"> {
...
export async function basicCommands(
  ...
  if (
    !addScript("types", ["typecheck", "type-check", "check:types", "types"], "general") &&
    detections.get("typescript")?.activeCapabilities.typeChecking &&
    [...context.files].some((file) => /(^|\/)tsconfig(?:\.[^/]+)?\.json$/.test(file))
  ) {
    commands.push({
      provider: "typescript",
      name: "TypeScript",
      category: "types",
      command: await expectedLocalBinary(context.root, "tsc"),
  ...
  if (!addScript("lint", ["lint", "check:lint", "lint:check"], "general")) {
    for (const [id, name, args] of [
      ...
      if (detections.get(id)?.activeCapabilities.linting)
        commands.push({
          provider: id,
          name,
          category: "lint",
          command: await expectedLocalBinary(context.root, id),
          args,
          ...withTimeout,
        });
    }
  ...
  if (!addScript("format", ["format:check", "check:format", "format-check"], "format")) {
    if (detections.get("prettier")?.activeCapabilities.formatting)
      commands.push({
        ...
        name: "Prettier",
        category: "format",
        command: await expectedLocalBinary(context.root, "prettier"),
        args: ["--check", "."],
        ...withTimeout,
      });
      ...
      commands.push({
        ...
        name: "Oxfmt",
        category: "format",
        command: await expectedLocalBinary(context.root, "oxfmt"),
        args: ["--check", "."],
        ...withTimeout,
      });
    else if (detections.get("biome")?.activeCapabilities.formatting)
      commands.push({
        provider: "biome-format",
        name: "Biome format",
        category: "format",
        command: await expectedLocalBinary(context.root, "biome"),
        args: ["format", "."],
        ...withTimeout,
      });
  }
  if (!addScript("tests", ["test", "test:run", "check:test"], "test")) {
    for (const [id, name, args] of [
      ...
      if (detections.get(id)?.activeCapabilities.testing)
        commands.push({
          provider: id,
          name,
          category: "tests",
