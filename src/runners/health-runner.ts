...
function severityAtLeast(severity: FindingSeverity, threshold: FindingSeverity): boolean {
  return ({ info: 0, warning: 1, error: 2 } as Record<FindingSeverity, number>)[severity] >= ({ info: 0, warning: 1, error: 2 } as Record<FindingSeverity, number>)[threshold];
}
...
function finalizeResults(results: HealthResult[], audit: AuditModel, config: RepnixConfig, options: RunHealthOptions, logger: DiagnosticLogger): HealthRun {
  ...
  return {
    ...
    repository: { root: audit.context.root, packageManager: audit.context.packageManager, kinds: audit.context.kinds, frameworks: audit.context.frameworks, languages: audit.context.languages, ...(audit.context.workspaceRoots ? { workspaces: audit.context.workspaceRoots } : {}), scopes: audit.context.scopes.map((scope) => ({ path: scope.path, roles: scope.roles })), ...(audit.registry ? { categories: audit.registry.categories.map((category) => ({ id: category.id, label: category.label, description: category.description })) } : {}) },
    summary: { status: exitCode === 2 ? "error" : exitCode === 1 ? "findings" : "healthy", findings: findings.length, newFindings, existingFindings: findings.length - newFindings, resolvedFindings, errors, exitCode },
    results,
  };
}

export async function runHealth(audit: AuditModel, config: RepnixConfig, options: RunHealthOptions = {}): Promise<HealthRun> {
  ...
  if (!audit.context.packageManager || audit.context.diagnostics.some((item) => item.severity === "error")) {
    results.push({ provider: "repnix", name: "RepNix configuration", category: options.category ?? "types", status: "error", findings: [], durationMs: 0, message: audit.context.diagnostics.find((item) => item.severity === "error")?.message ?? "Package manager unresolved" });
    return finalizeResults(results, audit, config, options, logger);
  }
  for (const coverage of audit.coverage) {
    if ((!options.category || options.category === coverage.category) && (categoryModeFor(config, coverage.category) === "required" || coverage.scopes.some((scope) => categoryModeFor(config, coverage.category, scope) === "required")) && coverage.status !== "covered") {
      results.push({ provider: "repnix", name: "Required coverage", category: coverage.category, status: "error", findings: [], durationMs: 0, message: `Required category '${coverage.category}' has no active provider.` });
    }
  }
  const tasks = await planHealthTasks(audit, config, { ...(options.category ? { category: options.category } : {}), timeoutMs, logger });
  const instrumented = tasks.map((task) => ({ ...task, run: async (completed: ReadonlyMap<string, HealthResult>) => {
    logger.info("health.provider.start", `Running ${task.name}`, { provider: task.provider, category: task.category });
    const result = await task.run(completed);
    logger.info("health.provider.finish", `Finished ${task.name}`, { provider: task.provider, category: result.category, status: result.status });
    return result;
  } }));
