...
const builtinDefinitions: Omit<CategoryDefinition, "label" | "description">[] = [
  ...
  { id: "architecture", requiredCapabilities: ["architectureRules"], applicable: (context) => matchingScopes(context, (scope) => (scope.productionSourceFiles ?? scope.sourceFiles).length >= 2, (scope) => `${scope.path} contains a module graph`) },
  ...
  { id: "documentation", requiredCapabilities: ["documentation"], applicable: (context) => ({ applicable: [...context.files].some((file) => /\.md$/i.test(file)), scopes: ["."], evidence: ["Markdown documentation detected"] }) },
...
export function categoryDefinition(category: HealthCategory, registry: Map<string, CategoryDefinition> = CATEGORY_REGISTRY): CategoryDefinition {
