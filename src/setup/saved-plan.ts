...
function isSelection(value: unknown): value is InstallPlanSelection {
  ...
  return (
    ...
    selection.providers.every((provider) => typeof provider === "string") &&
    typeof selection.includeCi === "boolean"
  );
}
...
export function parseSavedInstallPlan(value: unknown): SavedInstallPlan {
  ...
  if (
    ...
    throw new Error(
      "Invalid or outdated RepNix setup plan. Generate a new plan with `repnix setup --plan --format json`.",
    );
  }
...
export function assertSavedPlanMatches(saved: SavedInstallPlan, regenerated: InstallPlan): void {
  ...
  if (!isDeepStrictEqual(saved, expected)) {
    throw new Error(
      "Saved setup plan no longer matches this repository. Generate and review a new plan before applying it.",
    );
  }
}
