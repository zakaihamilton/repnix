...
try {
  ...
  await writeFile(
    path.join(consumer, "src", "app.jsx"),
  ...
  await writeFile(
    path.join(consumer, ".dependency-cruiser.cjs"),
    `module.exports = {\n  forbidden: [{\n    name: "no-source-to-test",\n    severity: "error",\n    from: { path: "^src" },\n    to: { path: "^test" },\n  }],\n};\n`,
  );
  await writeFile(
    path.join(consumer, ".size-limit.json"),
    `${JSON.stringify([{ path: "dist/bundle.js", limit: "1 B" }], null, 2)}\n`,
  );
  ...
  await run(
    ...
    { cwd: consumer, timeoutMs: 600_000 },
  );
  ...
  assert(
    audit.stdout.includes("Architecture boundaries     ✓ dependency-cruiser"),
    `Audit did not credit configured dependency-cruiser rules.\n${audit.stdout}`,
  );
  assert(
    audit.stdout.includes("Bundle regression           ✓ Size Limit"),
    `Audit did not credit a configured Size Limit budget.\n${audit.stdout}`,
  );

  const architecture = await run(bin, ["check", "architecture", "--format", "json"], {
    cwd: consumer,
    allowExitCodes: [1],
    timeoutMs: 600_000,
  });
  ...
  assert(
    architectureReport.results?.some(
      (result) => result.provider === "dependency-cruiser" && result.findings.length > 0,
    ),
    "dependency-cruiser did not produce a normalized violation.",
  );

  const bundle = await run(bin, ["check", "bundle", "--format", "json"], {
    cwd: consumer,
    allowExitCodes: [1],
    timeoutMs: 600_000,
  });
  ...
  assert(
    bundleReport.results?.some((result) => result.provider === "size-limit" && result.findings.length > 0),
    "Size Limit did not produce a normalized budget finding.",
  );
