...
try {
  ...
  assert(
    setup.includes("Repository health setup complete"),
    `Interactive package-health setup did not complete.\n${setup}`,
  );
  ...
  assert(
    audit.stdout.includes("Package publishing          ✓ Publint, Are The Types Wrong?"),
    `Audit did not credit both package-health providers after setup.\n${audit.stdout}`,
  );

  const check = await run(bin, ["check", "package-health", "--format", "json"], {
    cwd: consumer,
    allowExitCodes: [1],
    timeoutMs: 600_000,
  });
  ...
  assert(
    report.results?.some((result) => result.provider === "publint" && result.findings.length > 0),
    "Publint did not produce a normalized package finding.",
  );
  assert(
    report.results?.some((result) => result.provider === "attw" && result.findings.length > 0),
    "Are The Types Wrong? did not produce a normalized type-resolution finding.",
  );
  assert(
    report.results.every((result) => result.category === "package-health"),
    "Category filtering included an unrelated result.",
  );

  const details = await run(bin, ["check", "package-health", "--details"], {
    cwd: consumer,
    allowExitCodes: [1],
    timeoutMs: 600_000,
  });
