...
describe("provider normalizers", () => {
  ...
  it("normalizes jscpd clone pairs", () => {
    const findings = normalizeJscpd({
      duplicates: [
        { lines: 27, firstFile: { name: "src/a.ts", start: 2 }, secondFile: { name: "src/b.ts", start: 8 } },
      ],
    });
        ...
        violations: [
          {
            ...
            from: "src/app.ts",
            to: "test/helper.ts",
            rule: { name: "no-source-to-test", severity: "error" },
          },
        ],
      },
    });
      ...
      messages: [
        {
          ...
          path: ["exports", "."],
          args: {},
          formatted: 'pkg.exports["."] points to a missing file.',
        },
      ],
    });
  ...
  it("normalizes ATTW problems and packages without declarations", () => {
    ...
    expect(normalizeAttw({ analysis: { packageName: "demo", packageVersion: "1.0.0" } })[0]).toMatchObject({
