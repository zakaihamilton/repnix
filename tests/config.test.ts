...
describe("configuration", () => {
  it("provides zero-config defaults and accepts partial category maps", async () => {
    ...
    try {
      ...
      await writeFile(path.join(root, "repnix.config.json"), JSON.stringify({ schemaVersion: 1, categories: { "dead-code": { mode: "required" } }, severityThreshold: "error" }));
      expect((await readConfig(root)).config).toMatchObject({ categories: { "dead-code": { mode: "required" } }, severityThreshold: "error" });
    } finally {
  ...
  it("rejects removed provider enablement", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "repnix-config-"));
    try {
      await writeFile(path.join(root, "repnix.config.json"), JSON.stringify({ providers: { "osv-scanner": { enabled: false } } }));
