...
async function runCli(
  ...
  return await new Promise((resolve) => {
    const child = spawn(process.execPath, [path.join(projectRoot, "dist", "cli.js"), ...args], {
      cwd: root,
      env: { ...process.env, ...env },
    });
    let stdout = "";
...
describe("CLI", () => {
  ...
  it("keeps JSON on stdout while streaming verbose diagnostics to stderr", async () => {
    ...
    manifest.scripts.test =
      "node -e \"process.stdout.write('provider stdout\\n'); process.stderr.write('provider stderr\\n')\"";
  ...
  it("supports quiet mode and structured debug records", async () => {
    ...
    const structured = await runCli(root, [
      "check",
      "tests",
      "--format",
      "json",
    ...
    const records = structured.stderr
      .trim()
      .split("\n")
  ...
  it("keeps CLI argument errors structured when JSON diagnostics are requested", async () => {
    ...
    expect(records).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ event: "cli.error", message: expect.stringContaining("Invalid log level") }),
      ]),
    );
  });
  ...
  it("uses exit 1 for valid health findings", async () => {
    ...
    const result = await runCli(root, ["check", "--format", "json"]);
    const report = JSON.parse(result.stdout) as {
      summary: { exitCode: number; findings: number };
      results: Array<{ findings: Array<{ metadata?: { command?: string } }> }>;
    };
    expect(result.code).toBe(1);
    expect(report.summary).toMatchObject({ exitCode: 1, findings: 1 });
    expect(
      report.results.flatMap((item) => item.findings).some((finding) => finding.metadata?.command?.includes("node -e")),
    ).toBe(true);
  });
  ...
  it("uses exit 2 when required coverage is unavailable", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ schemaVersion: 1, categories: { security: { mode: "required" } } }),
    );
    const result = await runCli(root, ["check", "--format", "json"]);
  ...
  it("fails clearly when a repository command exceeds the configured timeout", async () => {
    ...
    const report = JSON.parse(result.stdout) as {
      summary: { exitCode: number; errors: number };
  ...
  it("runs OSV-Scanner offline and accepts nonzero exits with valid findings", async () => {
    ...
    expect(report.results).toContainEqual(
      expect.objectContaining({
        ...
        findings: [expect.objectContaining({ type: "vulnerability" })],
      }),
    );
  });
  ...
  it("runs configured dependency-cruiser and Size Limit providers", async () => {
    ...
    await writeFile(
      ...
      `module.exports = { forbidden: [{ name: "no-source-to-test", severity: "error", from: {}, to: {} }] };\n`,
    );
    await fakeBinary(
      root,
      "depcruise",
      `process.stdout.write(JSON.stringify({ summary: { violations: [{ from: "src/App.tsx", to: "test/helper.ts", rule: { name: "no-source-to-test", severity: "error" } }] } }));`,
    );
    await fakeBinary(
      root,
      "size-limit",
      `process.stderr.write("Size limit has exceeded the configured limit"); process.exitCode = 1;`,
    );

    const architecture = await runCli(root, ["check", "architecture", "--format", "json"]);
    const architectureReport = JSON.parse(architecture.stdout) as {
      summary: { exitCode: number };
    ...
    const bundleReport = JSON.parse(bundle.stdout) as {
      summary: { exitCode: number };
      results: Array<{ provider: string }>;
    };
  ...
  it("classifies the RepNix package as a CLI/library without browser recommendations", async () => {
    ...
    expect(
      report.coverage.filter((entry) => ["bundle", "accessibility", "performance"].includes(entry.category)),
    ).toEqual([
    ...
    expect(report.recommendations.some((item) => ["size-limit", "jsx-a11y", "lhci"].includes(item.provider))).toBe(
      false,
    );
  });
  ...
  it("records current debt in a baseline and fails only on new findings", async () => {
    ...
    expect(JSON.parse(await readFile(path.join(root, "repnix.config.json"), "utf8"))).toMatchObject({
    ...
    expect(JSON.parse(existing.stdout)).toMatchObject({
      summary: { findings: 1, newFindings: 0, existingFindings: 1, resolvedFindings: 0, exitCode: 0 },
    });
    ...
    expect(JSON.parse(regression.stdout)).toMatchObject({
      summary: { findings: 2, newFindings: 1, existingFindings: 1, exitCode: 1 },
    });
  });
...
for (const file of ["repnix.config.json", ".repnix-baseline.json"]) {
  ...
  it("emits SARIF and a serializable non-interactive setup plan", async () => {
    ...
    expect(JSON.parse(sarif.stdout)).toMatchObject({
      version: "2.1.0",
      runs: [{ tool: { driver: { name: "RepNix" } }, results: [expect.objectContaining({ baselineState: "new" })] }],
    });
    ...
    expect(JSON.parse(planned.stdout)).toMatchObject({
      schemaVersion: 1,
      selection: { providers: ["knip", "jscpd", "c8"], includeCi: false },
      files: expect.arrayContaining([expect.objectContaining({ path: "repnix.config.json" })]),
    });
  });
  ...
  it("normalizes TypeScript command output into located, actionable findings", async () => {
    ...
    await fakeBinary(
      root,
      "tsc",
      `process.stdout.write("src/index.ts(3,7): error TS2322: Type 'string' is not assignable to type 'number'.\\n"); process.exitCode = 2;`,
    );
    ...
    expect(result.code).toBe(1);
    expect(report.results.flatMap((entry) => entry.findings)).toContainEqual(
      expect.objectContaining({
        ruleId: "TS2322",
  ...
  it("normalizes absolute finding paths against the repository root from a workspace directory", async () => {
    ...
    await fakeBinary(
      root,
      "tsc",
      "process.stdout.write(`${process.cwd()}/packages/a/src/index.ts(3,7): error TS2322: bad\\n`); process.exitCode = 2;",
    );
